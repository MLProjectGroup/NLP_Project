...
def app():
    ...
    # --- Load uploaded CVs if available ---
    if "all_cv_chunks" in st.session_state and st.session_state.all_cv_chunks:
        rag_engine.add_cvs(st.session_state.all_cv_chunks)
        st.info(f"✅ Loaded {len(st.session_state.all_cv_chunks)} CV chunks from Upload page.")
    else:
        st.warning("⚠️ No CVs uploaded yet. Please go to Upload CVs page first!")
